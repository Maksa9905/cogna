...
<template>
  <form class="login-form" @submit.prevent="handleSubmit">
    <form.Field name="email" :validators="{ onSubmit: ({ value }) => validators.email(value) }">
      <template v-slot="{ field }">
        <UFormField :error="field.state.meta.errors[0]" :name="field.name" :label="t('common.email')">
          <UInput :icon="LetterIcon" size="xl" class="w-full" :model-value="field.state.value"
            @update:model-value="(field.handleChange)" placeholder="example@yandex.ru" />
        </UFormField>
      </template>
    </form.Field>
    ...
    <form.Field name="password" :validators="{ onSubmit: ({ value }) => validators.password(value) }">
      <template v-slot="{ field }">
        <UFormField :name="field.name" :error="field.state.meta.errors[0]" :label="t('common.password')">
          <PasswordInput :icon="LockIcon" :model-value="field.state.value" @update:model-value="field.handleChange"
            class="w-full" size="xl" :placeholder="t('auth.login.passwordPlaceholder')" />
        </UFormField>
      </template>
    </form.Field>
    ...
    <UButton :is-loading="isLoading" class="justify-center" size="xl" color="primary" type="submit">{{
      t('auth.login.submit') }}
    </UButton>
  </form>
</template>
